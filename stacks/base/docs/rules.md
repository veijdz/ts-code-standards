...
3. `internal` — first-party modules referenced by alias (`@/lib/users`).
...
**Exceptions.** _None._ The compiler-level enforcement (`verbatimModuleSyntax`) leaves no slack here.
...
#### Rule: `lodash`, `lodash-es`, `moment`, `querystring` (and `node:querystring`) are forbidden
