...
This is **not** an npm package — consumers copy the templates via `npx degit` and own their own dependencies. Those that need framework-specific opinions (e.g., a backend framework, a mobile bundler, a meta-framework) derive their own repo from this baseline and overlay rules on top.
