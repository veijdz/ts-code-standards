...
9. **Performance is a feature.**
   N+1 queries, blocking calls inside hot paths, and bundle bloat are not optimizations to consider later — they are bugs to fix at the same time as correctness. This is not about micro-optimization (`x++` vs `++x`); it is about catching the predictable, structural costs that are obvious in review and painful to fix in production. Profile before optimizing anything that does not fall in this category.
